...
export default function Signup() {
  ...
  // Simple OAuth availability check (optional)
  useEffect(() => {
    const checkOAuth = async () => {
      try {
        const response = await fetch('/api/auth/oauth-status?simple=true', {
          method: 'GET',
        ...
        if (response.ok && response.headers.get('content-type')?.includes('application/json')) {
          ...
          const hasOAuth = data.google || data.microsoft || data.linkedin;
          setShowSocialLogin(hasOAuth);
        }
      } catch (error) {
        console.log('OAuth check failed, social signup disabled');
        // Don't show error to user, just disable social login
      }
    };
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        {step === 'signup' && (
          <>
            ...
            {/* Social Signup Buttons - Only show if available */}
            {showSocialLogin && (
              <>
